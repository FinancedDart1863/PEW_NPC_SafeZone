...
namespace RadarBlock
{
	public class RadarProcess
	{
		...
		private static bool ProcessRadarItem(IMyEntity entity)
		{
			...
			IMyCubeGrid beaconGrid = beacon.CubeGrid;
			
			if(!(beaconGrid.IsStatic))
				return false;
...
//					LCDManager.Add(parent, radarFilter.OutputLCDName, output, m_LCDClear);
				...
				if (true) {
					...
					bool isEntirelyNPC = false;
					
					if (radarType == RadarObjectTypes.Ship)
					{
						IMyCubeGrid gridHandler = entity as IMyCubeGrid;

						
                        foreach (long owner in gridHandler.BigOwners)
                        {
		...
		private static RadarFilter GetRadarEntityFilters(IMyEntity radarEntity)
		{
			...
			radarBeacon.CustomData="-- This block is not configurable\n Range is hardcoded to 6 km.";

			filter.NoAsteroids = true;
			filter.NoShips = false;
			filter.NoStations = true;
			filter.NoCharacters = true;
			filter.NoHud = true;
			filter.PassengerHud = true;
			filter.OnlyPowered = true;
			filter.SonarMode = false;
